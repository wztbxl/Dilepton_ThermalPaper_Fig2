bl_info = {
    "name": "Import TOF Hits",
    "author": "Node Spaghetti_zhen",
    "version": (0, 0, 1),
    "blender": (2, 93, 0),
...
#    )
    ...
    def execute(self, context):
        ...
        hits = itextFile.readlines()
        name_num = 0
        name = 'hits.'

        if (hits):
            for hit in hits:
                pointlist = list(map(float, hit.split()))
                point = Vector(((pointlist[0]+1)*scaling_factor, (pointlist[1]+1)*scaling_factor, (pointlist[2]+1)*scaling_factor))
                radius = 3/10
                ...
                # bmesh.ops.create_icosphere(bm, subdivisions=2, radius=radius, matrix = Matrix.Translation(point) )
                bmesh.ops.create_cube(bm, size=radius, matrix = Matrix.Translation(point) )
                bm.to_mesh(mesh)
                
                obMesh.color = 1, 0, 0, 1.0
